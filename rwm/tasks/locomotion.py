...
class WalkTask(BaseTask):
    ...
    def get_observation(self, action= None):
        ...
        sensor_obs = self.get_sensor_data(self.data)
        prio_obs = self.get_pos_vel_trq(self.data)

        state = np.concatenate([sensor_obs, prio_obs], dtype= np.float32)
        priv_state = self.get_privileged_obs(self.model, self.data)
        aux_state = self.get_auxilary_obs(action, self.data)
    ...
    def reset(self):
        ...
        if self.CONTACT_BODY_IDS is None:
            self.CONTACT_BODY_IDS = self.get_contact_body_ids()

        observation = self.get_observation()

        if self._obs_dim is None:
            self._obs_dim = observation["state"].shape[0]

        return self.flatten_observation(observation)
    
    def step(self, action= None, imagination= None):

        if self.CONTACT_BODY_IDS is None:
            self.CONTACT_BODY_IDS = self.get_contact_body_ids(self.model)
        
        observation = self.get_observation(action)
        reward = self.compute_reward(observation, imagination, action)
        done = self.is_terminated(self.data)

        return self.flatten_observation(observation), reward, done
    ...
    def is_terminated(self, data) -> bool:
    ...
    def _reward_linear_vel_tracking(self, observation, imagination, temperature = 0.25):
        linear_vel= observation["state"][0:3]
        expected_linear_vel = imagination[0:3]
    ...
    def _reward_ang_vel_tracking(self, observation, imagination, temperature = 0.25):
        ...
        ang_vel = observation["state"][3:6]
        expected_ang_vel = imagination[3:6]
    ...
    def _reward_linear_vel_z(self, observation):
        ...
        linear_vel_z_reward = observation["state"][0:3][-1] ** 2
        return linear_vel_z_reward
    
    def _reward_ang_vel_xy(self, observation):

        ang_vel_xy_reward = np.linalg.norm(observation["state"][3:6], ord=2) ** 2
        return ang_vel_xy_reward
    
    def _reward_joint_torque(self, observation):

        torques_reward = np.linalg.norm(observation["state"][9+ 2*self.model.nu: 9+ 3*self.model.nu], ord=2) ** 2
        return torques_reward
    
    def _reward_joint_accl(self, observation):

        accl_reward = np.linalg.norm(observation["aux_state"][self.model.nu: 2*self.model.nu], ord=2) ** 2
        return accl_reward
    
    def _reward_action_rate(self, observation, action):

        assert observation["aux_state"][:self.model.nu].shape == action.shape

        action_reward = np.linalg.norm(observation["aux_state"][:self.model.nu] - action, ord=2) ** 2
        return action_reward
    
    def _reward_joint_deviation(self, observation):
        assert observation["state"][9: 9+ self.model.nu].shape == self.default_qpos.shape

        devation_reward = np.linalg.norm(observation["state"][9: 9+ self.model.nu] - self.default_qpos, ord=2) ** 2
    ...
    def observation_space(self):
        ...
        if self._obs_dim is None:
            # safe fallback
            return gym.spaces.Box(low=-np.inf, high=np.inf, shape=(128,))
